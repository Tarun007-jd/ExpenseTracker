    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div>
                ...
                <p className="text-sm text-dark-900 dark:text-dark-500 mt-1">Insights into your spending patterns</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="glass-card p-5 animate-slide-up" style={{ animationDelay: '0ms' }}>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-medium text-dark-900 dark:text-dark-400">Spending Trend</span>
                    </div>
                    ...
                    <p className="text-xs text-dark-900 mt-1">vs last month</p>
                </div>
                ...
                <div className="glass-card p-5 animate-slide-up" style={{ animationDelay: '100ms' }}>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-medium text-dark-900 dark:text-dark-400">Daily Average</span>
                    </div>
                    <p className="text-xl font-bold text-dark-900 dark:text-dark-100">₹{dailyAvg.toLocaleString()}</p>
                    <p className="text-xs text-dark-900 mt-1">this month</p>
                </div>
                ...
                <div className="glass-card p-5 animate-slide-up" style={{ animationDelay: '200ms' }}>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm font-medium text-dark-900 dark:text-dark-400">Biggest Category</span>
                    </div>
                    <p className="text-xl font-bold text-dark-900 dark:text-dark-100">{topCategory?.name || '-'}</p>
                    <p className="text-xs text-dark-900 mt-1">₹{(topCategory?.value || 0).toLocaleString()}</p>
                </div>
            </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    ...
                                    <Legend iconType="circle" iconSize={8}
                                        formatter={(value) => <span className="text-xs text-dark-900 dark:text-dark-400">{value}</span>}
                                    />
                                </PieChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="flex items-center justify-center h-full text-dark-900 text-sm">No data yet</div>
                        )}
                    </div>
                </div>
            ...
            <div className="glass-card p-5">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-dark-900 mt-1">Based on the last {monthlyData.length} months</p>
                    </div>
                    <div className="text-right">
                        <p className="text-3xl font-bold text-gradient">₹{monthlyAvg.toLocaleString()}</p>
                        <p className="text-xs text-dark-900 mt-1">per month</p>
                    </div>
                </div>
            </div>
        </div>
