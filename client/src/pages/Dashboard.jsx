        ...
        if (active && payload && payload.length) {
            return (
                <div className="glass-card p-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)] !rounded-xl text-sm border-none bg-white/90 dark:bg-dark-800/90 backdrop-blur-md">
                    <p className="font-extrabold text-dark-900 dark:text-dark-200">{label}</p>
    ...
    return (
        ...
        >
            <motion.div variants={itemVariants}>
                <h2 className="text-4xl font-black text-dark-900 dark:text-dark-100 tracking-tight">Dashboard</h2>
                <p className="text-base text-dark-900 dark:text-dark-400 mt-2 font-medium">An overview of your financial activity</p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <motion.div variants={itemVariants} className="lg:col-span-2 glass-card p-6 shadow-sm bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl">
                    <h3 className="text-lg font-extrabold text-dark-900 dark:text-dark-200 mb-6 tracking-tight">Monthly Spending</h3>
                    <div className="h-80">
                        {monthlyData.length > 0 ? (
                            ...
                            <div className="flex items-center justify-center h-full text-dark-900 dark:text-dark-500 font-medium">
                ...
                <motion.div variants={itemVariants} className="glass-card p-6 shadow-sm bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl">
                    <h3 className="text-lg font-extrabold text-dark-900 dark:text-dark-200 mb-6 tracking-tight">By Category</h3>
                    <div className="h-80">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    ...
                                    <Legend
                                        ...
                                        formatter={(value) => <span className="text-sm font-semibold text-dark-900 dark:text-dark-400">{value}</span>}
                                    />
                                    <Tooltip
            ...
            {/* Recent Expenses */}
            <motion.div variants={itemVariants} className="glass-card p-6 shadow-sm bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl">
                <h3 className="text-lg font-extrabold text-dark-900 dark:text-dark-200 mb-6 tracking-tight">Recent Expenses</h3>
                  ...
                  hover:bg-primary-50/50 transition-colors cursor-default border border-transparent hover:border-primary-100"
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-base font-bold text-dark-900 dark:text-dark-200">{exp.category}</p>
                                        <p className="text-sm text-dark-900 dark:text-dark-400 font-medium">{exp.description || 'No description'}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-lg font-black text-dark-900 dark:text-dark-100">₹{exp.amount.toLocaleString()}</p>
                                    <p className="text-sm text-dark-900 dark:text-dark-500 font-medium">{new Date(exp.date).toLocaleDateString()}</p>
                                </div>
                            </motion.div>
                        ))}
                    </motion.div>
                ) : (
                    <p className="text-center text-dark-900 dark:text-dark-500 font-medium py-8">No expenses recorded yet</p>
                )}
            </motion.div>
        </motion.div>
    );
