        ...
        >
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-base text-slate-500 dark:text-slate-400 font-medium mt-2">{total} total expenses recorded</p>
                </div>
            ...
            <motion.div variants={itemVariants} className="glass-card p-5 space-y-4 shadow-sm bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl">
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="relative flex-1">
                        <HiOutlineSearch className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-slate-500" />
                        <input
                            type="text"
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            placeholder="Search expenses..."
                            className="w-full pl-12 pr-4 py-3 rounded-2xl border border-slate-200 dark:border-dark-700
                bg-slate-50 dark:bg-dark-900 text-slate-900 dark:text-slate-100 font-medium
                ...
                outline-none transition-all text-sm"
                    ...
                    <select
                        ...
                        className="px-5 py-3 rounded-2xl border border-slate-200 dark:border-dark-700 font-medium
              bg-slate-50 dark:bg-dark-900 text-slate-900 dark:text-slate-100
              focus:ring-2 focus:ring-primary-500 focus:border-transparent focus:bg-white
              ...
              border transition-all ${showFilters
                                ...
                                : 'border-slate-200 dark:border-dark-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50'}`}
                    >
                ...
                {showFilters && (
                    ...
                    >
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">From</label>
                            <input
                                type="date"
                                value={startDate}
                                onChange={(e) => setStartDate(e.target.value)}
                                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-dark-700 font-medium
                  bg-slate-50 dark:bg-dark-900 text-slate-900 dark:text-slate-100
                  focus:ring-2 focus:ring-primary-500 outline-none transition-all text-sm"
                            />
                        </div>
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">To</label>
                            <input
                                type="date"
                                value={endDate}
                  ...
                  focus:ring-2 focus:ring-primary-500 outline-none transition-all text-sm"
                        ...
                        <button
                            ...
                            className="self-end px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-danger-600 hover:bg-danger-50 transition-colors"
                        >
                            Clear dates
                        </button>
                    </motion.div>
            ...
            <motion.div variants={containerVariants} className="space-y-3">
                {loading ? (
                    ...
                    <motion.div variants={itemVariants} className="glass-card p-16 text-center shadow-sm bg-white/80 dark:bg-dark-800/80">
                        ...
                        <p className="text-slate-400 dark:text-slate-500 font-medium text-sm mt-2">Try adjusting your filters or add a new expense</p>
                    </motion.div>
                ) : (
                    expenses.map((exp) => (
                        <motion.div
                            ...
                            <div className="flex items-center gap-4 flex-1 min-w-0">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="px-2.5 py-1 rounded-md bg-slate-100 text-xs font-bold text-slate-500 dark:text-slate-400">
                                            {new Date(exp.date).toLocaleDateString()}
                                        </span>
                                    </div>
                                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400 truncate mt-1">{exp.description || 'No description'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 ml-4">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="p-2.5 rounded-xl hover:bg-primary-50 text-slate-400 dark:text-slate-500 hover:text-primary-600 transition-colors"
                                    >
                                        <HiOutlinePencil className="w-5 h-5" />
                                    </button>
                                    <button
                                        onClick={() => handleDelete(exp._id)}
                                        className="p-2.5 rounded-xl hover:bg-danger-50 text-slate-400 dark:text-slate-500 hover:text-danger-600 transition-colors"
                                    >
