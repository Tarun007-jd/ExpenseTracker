...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    return (
        <>
          ...
          transition-transform duration-300 ease-in-out border-r border-slate-100 dark:border-dark-700
          lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
            >
                {/* Header */}
                <div className="p-6 border-b border-slate-100 dark:border-dark-700 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-black text-slate-900 dark:text-slate-100 tracking-tight">ExpenseTracker</h1>
                        </div>
                    </div>
                ...
                <nav className="flex-1 p-5 space-y-2">
                  ...
                  ${isActive
                                        ...
                                        : 'text-slate-500 dark:text-slate-400 hover:bg-primary-50 hover:text-primary-600 hover:translate-x-1'
                                    }`}
                            >
                ...
                {/* Bottom section */}
                <div className="p-5 border-t border-slate-100 dark:border-dark-700 space-y-3">
                    {/* Theme toggle */}
                    <button
                        onClick={toggleTheme}
                        className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold
              text-slate-500 dark:text-slate-400 hover:bg-slate-100 transition-all duration-200"
                    >
                    ...
                    {/* User info */}
                    <div className="flex items-center gap-3 px-4 py-2 mb-2 bg-slate-50 dark:bg-dark-900 rounded-2xl border border-slate-100 dark:border-dark-700">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-extrabold text-slate-800 dark:text-slate-200 truncate">{user?.name}</p>
